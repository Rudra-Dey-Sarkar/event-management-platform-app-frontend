...
function LoginAsGuest(setIsPresent:any, setIsActive:any){
  const guestUser = [{
    email:"guest"
  }]
  
  setCookie("user", guestUser);
  setIsPresent(true);
  setIsActive(false);
}
async function LoginUser(data: UserDataType, setIsPresent: any, setIsLoading: React.Dispatch<React.SetStateAction<boolean>>, setIsActive: any) {
  setIsLoading(true);
  try {
    ...
    setIsLoading(false);
    console.log("Cannot Proceed To Login Due To :-", errors);
...
function Login() {
  ...
  return (
    <form
      onSubmit={handleSubmit((data) => LoginUser(data, setIsPresent, setIsLoading, setIsActive))}
