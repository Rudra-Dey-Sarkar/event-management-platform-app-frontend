...
function OwnTodayEvents({ user, setIsModalOpen }: { user: any[] | UserDataType, setIsModalOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
            ...
            {events.length > 0 ?
                <div className='flex gap-x-4 py-2 px-3'>
                    {events?.map((event: EventDataType[0], index: number) =>
                        <div
                            className={`${CompareDateAndTime(event?.dateAndTime?.date, event?.dateAndTime?.time) === "today" ? "visible" : "hidden"}`}
                            key={index}>
                            {CompareDateAndTime(event?.dateAndTime?.date, event?.dateAndTime?.time) === "today" &&
