...
function ViewAll({ eventId }: { eventId: string | null }) {
    ...
    useEffect(() => {
        ...
        if (cookies !== undefined && typeof cookies === "string") {
            ...
            ViewEvent(userCookieData?.[0]?.email, setEvents);
            ...
            {isModalOpen === true &&
                <div
                    ...
                    <div
                        ...
                        onClick={(e) => e.stopPropagation()}>
                        <ViewAllEventModal user={user} setIsModalOpen={setIsModalOpen} eventId={eventId} />
                    </div>
                </div>
            }
            {events.length > 0 ?
                <div className='flex gap-x-4 py-2 px-3'>
                    {events?.map((event: EventDataType[0], index: number) =>
                        <div
                          className={`${CompareDateAndTime(event?.dateAndTime?.date, event?.dateAndTime?.time) === eventId ? "visible" : "hidden"}`}
                            key={index}>
                            {CompareDateAndTime(event?.dateAndTime?.date, event?.dateAndTime?.time) === eventId &&
